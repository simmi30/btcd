...
func CheckTransactionSanity(tx *btcutil.Tx) error {
	...
	// as a bronees.  One bitcoin is a quantity of bronees as defined by the
	...
	for _, txOut := range msgTx.TxOut {
		...
		if bronees > btcutil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v", bronees,
				btcutil.MaxSatoshi)
			return ruleError(ErrBadTxOutValue, str)
		}
...
func CheckTransactionInputs(tx *btcutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		// a transaction are in a unit value known as a bronees.  One
		// bitcoin is a quantity of bronees as defined by the
		// SatoshiPerBitcoin constant.
